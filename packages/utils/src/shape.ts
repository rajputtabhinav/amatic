...
import { invariant } from "@amatic/common";
import {
  curve,
  lineSegment,
  pointFrom,
...
import { getElementAbsoluteCoords } from "@amatic/element";
...
import type { Curve, LineSegment, Polygon, Radians } from "@amatic/math";
...
export const getCurveShape = <Point extends GlobalPoint | LocalPoint>(
  ...
  let p0 = pointFrom(0, 0) as Point;

  for (const op of ops) {
    if (op.op === "move") {
      const p = pointFromArray(op.data) as Point | undefined;
      invariant(p != null, "Ops data is not a point");
      p0 = transform(p);
    }
    if (op.op === "bcurveTo") {
      const p1 = transform(pointFrom(op.data[0], op.data[1]) as Point);
      const p2 = transform(pointFrom(op.data[2], op.data[3]) as Point);
      const p3 = transform(pointFrom(op.data[4], op.data[5]) as Point);
...
export const getClosedCurveShape = <Point extends GlobalPoint | LocalPoint>(
  ...
  roughShape: Drawable,
  startingPoint: Point = pointFrom(0, 0) as Point,
  angleInRadian: Radians,
  center: Point,
): GeometricShape<Point> => {
  const transform = (p: Point) =>
    pointRotateRads<Point>(
      pointFrom<Point>(p[0] + startingPoint[0], p[1] + startingPoint[1]),
      center,
      angleInRadian,
    );
...
export const ellipseExtremes = <Point extends LocalPoint | GlobalPoint>(
  ...
  return [
    ...
    vectorAdd(vectorScale(vector(xMax, yAtXMax), -1), centerVector),
  ];
};
