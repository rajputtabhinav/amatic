import { promiseTry, LOCAL_FONT_PROTOCOL } from "@amatic/common";
...
export class ExcalidrawFontFace {
  ...
  private static readonly ASSETS_FALLBACK_URL = `https://esm.sh/${
    import.meta.env.PKG_NAME
      ...
      : "@amatic/amatic" // fallback to the latest package version (i.e. for app)
  }/dist/prod/`;
  ...
  private static createUrls(uri: string): URL[] | DataURL[] {
    ...
    if (typeof window.AMATIC_ASSET_PATH === "string") {
      const normalizedBaseUrl = this.normalizeBaseUrl(window.AMATIC_ASSET_PATH);

      urls.push(new URL(assetUrl, normalizedBaseUrl));
    } else if (Array.isArray(window.AMATIC_ASSET_PATH)) {
      window.AMATIC_ASSET_PATH.forEach((path) => {
