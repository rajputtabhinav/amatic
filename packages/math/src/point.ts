 ...
 */
export function pointFrom<Point extends GlobalPoint | LocalPoint = GlobalPoint>(
  x: number,
  y: number,
): Point {
  return [x, y] as Point;
...
export function pointFromArray<
  Point extends GlobalPoint | LocalPoint = GlobalPoint,
>(numberArray: number[]): Point | undefined {
  return numberArray.length === 2
    ? (pointFrom<Point>(numberArray[0], numberArray[1]) as Point)
    : undefined;
}
...
export function pointFromVector<
  P extends GlobalPoint | LocalPoint = GlobalPoint,
>(v: Vector, offset?: P): P {
  const off = offset || (pointFrom<P>(0, 0) as P);
  return pointFrom<P>(off[0] + v[0], off[1] + v[1]) as P;
}
...
>(p: From, v: Vector = [0, 0] as Vector): To {
  return pointFrom(p[0] + v[0], p[1] + v[1]) as To;
}
...
export function pointCenter<P extends LocalPoint | GlobalPoint>(a: P, b: P): P {
  return pointFrom((a[0] + b[0]) / 2, (a[1] + b[1]) / 2) as P;
}
